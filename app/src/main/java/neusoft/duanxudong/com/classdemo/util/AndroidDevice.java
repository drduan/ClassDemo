 ...
 * https://github.com/android/platform_packages_apps_email/blob/master/
...
public class AndroidDevice {
    ...
    private static final String LOG_TAG = AndroidDevice.class.getName();
    ...
    static private String getDeviceIdInternal(Context context)
        ...
        if (context == null) {
            throw new IllegalStateException(
                    "getDeviceId requires a Context");
        }
        ...
        if (f.exists()) {
            if (f.canRead()) {
                ...
                Log.w(LOG_TAG, f.getAbsolutePath()
                        + ": File exists, but can't read?"
                        + "  Trying to remove.");
                if (!f.delete()) {
                    Log.w(LOG_TAG, "Remove failed. Tring to overwrite.");
                }
            }
        }
    ...
    /**
     * @return AndroidDevice's unique ID if available. null if the device has no
     * unique ID.
     */
